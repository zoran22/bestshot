...
// Head image upload
        ...
        $headImg = null;
    if (isset($_FILES["head_image"]["name"])) {
// Head image upload
        ...
        if ($_FILES["head_image"]['name'] != "" && ($_FILES["head_image"]["size"] < 2097152 && in_array($head_img_ext, $valid_head_ext))) {
    ...
    $postService = new \Service\PostService($db);
    if ($postService->updatePage($headImg, $galleryImgs)) {
...
if (isset($_GET['del'])) {
    ...
    $postService->delImage($name, $slug, $table);
    $data['postService'] = $postService->getSinglePost($slug);
    $data['gallery'] = $postService->getGallery($slug);
    // $app->loadTemplate('editTemplate', $data);
    header('Location: editpage.php?id=' . $slug);
    exit();
}
...
if (isset($_GET['id'])) {
    $data['postService'] = $postService->getSinglePost($_GET['id']);
    $data['gallery'] = $postService->getGallery($_GET['id']);
    // $data['post_id'] = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
    $app->loadTemplate('editTemplate', $data);
}
