$.extend({
	...
	*/
	toast: function(obj) {
        ...
        var fontsize = obj.fontsize || '12px';
		if(position == 'bottom') {
		...
		if(flag) {
            var content = "<div  id='toast-div' style='position: fixed;display: none; z-index:10000;font-size: " + fontsize + "; " + position + ";left: 0;width:100%; height: " + height + "; text-align: center'>";
		} else {
            var content = "<div id='toast-div' style='position: fixed; display: none;z-index:1000; top: 0; left: 0;width:100%; height:100%; text-align: center'>";
		}
        content += '<div class="z-depth-1" id="toast-content" style="display: inline-block; width: ' + width + ';min-height: ' + height + ';padding: 8px 14px;background-color: ' + backgroundColor + ';text-align: center;line-height: ' + lineheight + ';border-radius: 3px;color: ' + textColor + ';">' + title + '</div>';
		content += '</div>';
		$("body").append(content);
